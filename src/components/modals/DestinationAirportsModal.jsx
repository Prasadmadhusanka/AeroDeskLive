...
import styles from "../../styles/component_styles/modals/DestinationAirportsModal.module.css";

const DestinationAirportsModal = ({ show, onHide, data }) => {
  const [activeKey, setActiveKey] = useState(null);

  // Build mapping airport -> flights
  const airportsData = data.reduce((acc, flight) => {
    if (!flight.arrivalAirportIataCode || !flight.arrivalAirportName)
      return acc;

    const airportCode = flight.arrivalAirportIataCode;
    const airportName = flight.arrivalAirportName;
    const country = flight.arrivalAirportCountry;
    const countryCode = flight.arrivalAirportCountryCode;
    const region = flight.arrivalAirportRegion;
  ...
  const uniqueCountries = new Set(
    data
      .filter((f) => f.arrivalAirportCountry)
      .map((f) => f.arrivalAirportCountry)
  );
                          ...
                          <div className={styles.flightMain}>
                            ...
                            <div className={styles.flightDetails}>
                              ...
                              <div className={styles.detailGroup}>
                                ...
                                >
                                  {f.flightStatus?.toLowerCase() === "active"
                                    ? "Departed"
                                    : f.flightStatus}
                                </span>
                              </div>
...
export default DestinationAirportsModal;
